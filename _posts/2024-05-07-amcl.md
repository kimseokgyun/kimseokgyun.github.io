...
- t-1 단계에서 t 까지의 움직임을 총 3가지로 성분을 나눈다.
  ...
  - Local Odometry의 오차를 위와같이 3가지 성분으로 표현하여 파라미터를 통해 파티클을 어느정도 노이즈를 포함해서 이동시킬지 결정
...
- 위의 <U>5~7 Line</U> 에서 자세히 들여다보면 다음과 같은 수식으로 \\\(\hat{\delta}\\\)<sub>사후 추정치</sub> 를 구한다
  - 
