...
- 입력  \\\(u_t\\\) 에 따른  \\\(x_t-1\\\) 을 Simulate한다.
...
- dt 단위가 아닌 , v(이동거리), a(이동각도) 에 따라 파티클의 상태를 추정한다. 
  ...
  - 기본적으로 Odometry Motion Model은 dt에따라 엔코더값을 적분하는형태로 위치를 추종 (보통의 Local Odometry)

  - 해당 Local Odometry 기준으로 일정거리<sub>d_thresh_</sub>, 일정각도<sub>a_thresh_</sub> 이상 주행하면 파티클을 업데이트 한다는 의미

- t-1 단계에서 t 까지의 움직임을 총 3가지로 노이즈 성분을 나눠서 분산의 형태를 정한다.
  - 즉 t-1 단계에서 t까지 (극단적으로) S자형태로 곡선으로 움직이더라도 t-1와 t단계의 
  로봇 pose로 파티클의 분산을 결정한다는 의미


<br/>
<!-- --------------------------------------------------------------------------------
다음은 나의 뇌피셜입니다.
보통의 로봇 주행에서 회전 명령이 많이 들어가는경우는 
1. 출발할때 목표 지점 각도를 향해 많은 회전이 이루어짐
2. 도착후 목표 각도를 보상하기위해 많은 회전이 이루어짐
그러므로, 실제 t 사이의 모든 회전을 이 2가지  \\\(\delta_{rot1}\\\), \\\(\delta_{rot2}\\\) 로 나누어서 출발지점과 목표지점의 지면이 다르기 때문에 독립적인 노이즈 모델임
직진에서의 오차는 회전보다는 적은편이므로  \\\(\delta_{trans}\\\) 로 따로 변수를 만듦
-------------------------------------------------------------------------------- -->
<br/>
{% highlight c++ %}
  if (sqrt(
...
- 제자리 회전인 경우에는 \\\(\delta_{rot1}\\\),\\\(\delta_{rot2}\\\) 노이즈가 겹치므로 if 문으로 처리된걸 확인할수 있다.
