...
-  \\\(x_t-1\\\)에 해당하는 pf_odom_pose_ 와 현재상태 \\\(x_t\\\)에 해당하는 pose 의 delta를 계산한다
...
그러므로, 실제 t 사이의 모든 회전을 이 2가지  \\\(\delta_rot1\\\), \\\(\delta_rot2\\\) 로 나누어서 출발지점과 목표지점의 지면의 회전 노이즈 경향을 다르게 줌.
직진에서의 오차는 회전보다는 적은편이므로  \\\(\delta_trans\\\) 로 따로 변수를 만듦</del>
